...
export default class VariantPicker extends Component {
  ...
  variantChanged(event) {
    ...
    // Custom: Update product description for Sekiro: Shadows Die Twice Modded MEGA PACK
    const productTitle = document.querySelector('.view-product-title, h1, .product-title');
    let isSekiro = false;
    if (productTitle && typeof productTitle.textContent === 'string') {
      isSekiro = productTitle.textContent.trim().toLowerCase().includes('sekiro: shadows die twice modded mega pack');
    }
    if (isSekiro) {
      // Find the variant option (PS4/PS5)
      let selectedOption = '';
      const radio = document.querySelector('input[type="radio"][name*="PS"]:checked');
      if (radio && radio instanceof HTMLInputElement) {
        selectedOption = radio.value.trim().toLowerCase();
      } else {
        // Try select dropdown
        const select = document.querySelector('select');
        if (select && select instanceof HTMLSelectElement) {
          selectedOption = select.value.trim().toLowerCase();
        }
      }
      // Find the new description container
      const descBlock = document.querySelector('rte-formatter.text-block');
      if (descBlock && descBlock instanceof HTMLElement) {
        // Store the original description if not already stored
        if (!descBlock.dataset.originalDescription) {
          descBlock.dataset.originalDescription = descBlock.innerHTML;
        }
        if (selectedOption === 'ps5') {
          descBlock.innerHTML = '<p>i love abduallah</p>';
        } else {
          descBlock.innerHTML = descBlock.dataset.originalDescription;
        }
      }
    }
...
// --- Sekiro Description Swap Robust Handler ---
function applySekiroDescriptionSwap() {
  const productTitle = document.querySelector('.view-product-title, h1, .product-title');
  if (!productTitle || !productTitle.textContent) return;
  if (!productTitle.textContent.trim().toLowerCase().includes('sekiro: shadows die twice modded mega pack')) return;
...
// Run on initial load
applySekiroDescriptionSwap();

// Run every time the variant changes or the section is re-rendered
// Listen for select changes
if (typeof window !== 'undefined') {
  document.addEventListener('change', function(e) {
    if (e.target && e.target instanceof Element && e.target.tagName === 'SELECT') {
      setTimeout(applySekiroDescriptionSwap, 50); // slight delay for DOM update
    }
  });

  // Also use a MutationObserver in case Shopify replaces the DOM
